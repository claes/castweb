...
func TestParseNFO_OK(t *testing.T) {
    ...
    if title != "Strange Filters" { t.Fatalf("bad title: %q", title) }
    if plot == "" { t.Fatalf("empty plot") }
    if thumb == "" { t.Fatalf("empty thumb") }
    if len(tags) != 2 { t.Fatalf("want 2 tags, got %d", len(tags)) }
}
